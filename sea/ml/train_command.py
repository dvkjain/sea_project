...
def save_model(model, filename, scaler, target_scaler, scaling, target_scaling, y_train_unscaled, X_train_unscaled, mode):
    ...
    to_save['X_train_unscaled'] = X_train_unscaled
    to_save['mode'] = mode
...
@click.command()
@click.argument("path")
@click.option("--mode", type=click.Choice(["regression", "classification"]), default="regression", show_default=True, help="Specify the task type: regression or classification.")
...
def train(path, mode, scaling, target_scaling, epochs, batch_size, neurons_per_layer, learning_rate, activation, save):
    ...
    try:
        ...
        click.echo(f"\n{mode.upper()} model trained successfully on {path} with {len(X_train)} samples.")
        ...
        if save:
            save_model(model, save, scaler, target_scaler, scaling, target_scaling, y_train_unscaled, X_train_unscaled, mode)
