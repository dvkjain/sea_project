...
def show_metrics(metrics, y_test, y_pred_unscaled, X_test, mode):
    from sklearn.metrics import (
        mean_squared_error, mean_absolute_error, mean_absolute_percentage_error,
        r2_score, median_absolute_error, explained_variance_score, max_error,
        mean_squared_log_error, accuracy_score, precision_score, recall_score, f1_score
    )
    ...
    for metric in metrics.split(","):
        ...
        if metric in metric_functions:
            if mode == "classification":
                value = metric_functions[metric](y_test, y_pred_unscaled, average="weighted")
            else:
...
def evaluate(model_path, data_path, metrics):
    ...
    try:
        ...
        if mode == "classification":
            y_pred_unscaled = y_pred_scaled  # For classification, predictions are class labels
    ...
    if metrics:
        show_metrics(metrics, y_test, y_pred_unscaled, X_test, mode)
